...
def run_system_setup_skip_mid_wizard_check(dab_topic, test_name, tester, device_id):
    ...
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    result = TestResult(test_id, device_id, "system/setup/skip", "{}", "UNKNOWN", "", logs)
    user_validated_skip = "N/A"

    try:
        # Header / description
        for line in (
            f"[TEST] System_Setup Skip_Mid_Wizard Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: From a partially completed setup wizard, call system/setup/skip and verify that the device lands on the home screen with account features disabled.",
            "[DESC] Required operation: system/setup/skip. Factory reset and wizard navigation are handled manually or by other tests.",
            "[DESC] Pass criteria: system/setup/skip returns 200, and the tester confirms the device exits the wizard and shows the home screen without a signed-in account.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Capability gate: only require system/setup/skip
        if not require_capabilities(tester, device_id, "ops: system/setup/skip", result, logs):
            return result

        # Pre-step: confirm environment is really mid-wizard (manual)
        for line in (
            "[STEP] Manual precondition check — The device should currently be at a mid-setup wizard screen (e.g., after language selection, before network/account setup).",
            "[DESC] If needed, perform a factory reset and progress the wizard manually until DAB connectivity is restored and the device is reachable.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if not env_ok:
            result.test_result = "SKIPPED"
            line = "[RESULT] SKIPPED — Device is not in the required mid-setup wizard state for this test."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 1: Invoke system/setup/skip
        payload = "{}"
        line = f"[STEP] Invoking system/setup/skip with payload: {payload}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "system/setup/skip", payload, logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] system/setup/skip returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        # 501 → not implemented on this device
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/setup/skip is not implemented on this device (status=501)."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/setup/skip returned non-200 status={status}; cannot confirm skip behavior."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 2: Give the device time to transition to the home screen
        for line in (
            "[STEP] Waiting for the device to process system/setup/skip and exit the wizard.",
            "[DESC] Do not interact with the device during this wait; let the wizard complete the skip transition.",
        ):
            LOGGER.result(line)
            logs.append(line)

        # Use existing helper if available (countdown). This call assumes such a helper exists.
        # If countdown is not implemented in your codebase yet, see notes below for how to add it.
        countdown(result, logs, 45, "Waiting up to 45 seconds for the device to reach the home screen after system/setup/skip...")

        # Step 3: Manual validation on device UI
        for line in (
            "[STEP] Manual validation — Inspect the physical device/TV screen.",
            "[DESC] The device should have exited the setup wizard and be showing the Android TV home screen.",
            "[DESC] Account-based features (e.g., Google account, app sign-in) should remain disabled / not configured.",
        ):
            LOGGER.result(line)
            logs.append(line)

        user_validated_skip = yes_or_no(
            result,
            logs,
        ...
        if user_validated_skip:
            result.test_result = "PASS"
            line = "[RESULT] PASS — Tester confirmed that system/setup/skip skipped the remaining wizard steps and landed on the home screen with account features disabled."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — Tester reported that the device did NOT exit the wizard correctly or account configuration state did not match expectations."
        ...
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running system/setup/skip mid-wizard check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — An unexpected error occurred during system/setup/skip mid-wizard check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = (
            f"[SUMMARY] outcome={result.test_result}, user_validated_skip={user_validated_skip}, "
            f"test_id={test_id}, device={device_id}"
        )
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_system_setup_skip_initial_wizard_check(dab_topic, test_name, tester, device_id):
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] System_Setup Skip_Initial_Wizard Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: From the initial setup wizard screen after factory reset, call system/setup/skip and verify the device lands on the home screen with account features disabled.",
            "[DESC] Required operation: system/setup/skip. Factory reset is handled outside this test.",
            "[DESC] Pass criteria: system/setup/skip returns 200 and the tester confirms the setup wizard is bypassed and the home screen is shown without a signed-in account.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Manual precondition check: device must be at initial wizard
        for line in (
            "[STEP] Manual precondition — The device should be at the initial setup wizard screen immediately after a factory reset.",
            "[DESC] If needed, perform a factory reset and wait until the first setup screen appears and the device is reachable over DAB.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        if not env_ok:
            result.test_result = "SKIPPED"
            line = "[RESULT] SKIPPED — Device is not in the required initial setup wizard state for this test."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 1: Invoke system/setup/skip at initial wizard
        payload = "{}"
        line = f"[STEP] Invoking system/setup/skip at initial wizard with payload: {payload}"
        LOGGER.result(line)
        logs.append(line)
        ...
        # Step 2: Give the device time to transition and ask for manual verification
        for line in (
            "[STEP] Allow the device some time (≈30–60 seconds) to process system/setup/skip and exit the wizard.",
            "[DESC] Do not interact with the device during this period; let the wizard complete the skip transition.",
        ):
            LOGGER.result(line)
            logs.append(line)

        user_validated_home = yes_or_no(
            result,
            logs,
        ...
        if user_validated_home:
            result.test_result = "PASS"
            line = "[RESULT] PASS — Tester confirmed that system/setup/skip bypassed the setup wizard and landed on the home screen with account features disabled."
        else:
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — Tester reported that the device did NOT exit the wizard correctly or account state did not match expectations."
        ...
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running system/setup/skip initial wizard check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — An unexpected error occurred during system/setup/skip initial wizard check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = f"[SUMMARY] outcome={result.test_result}, user_validated_home={user_validated_home}, test_id={test_id}, device={device_id}"
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_system_power_mode_set_missing_mode_param_check(dab_topic, test_name, tester, device_id):
    ...
    Expected behavior:
      ...
      - Invalid call: system/power-mode/set with an empty JSON object {} should return 400 (Bad Request).
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] System_PowerMode_Set Missing_Mode_Param Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Verify that system/power-mode/set rejects a request missing the 'mode' field and that the power state remains Active.",
            "[DESC] Required operations: system/power-mode/get, system/power-mode/set.",
            "[DESC] Pass criteria: "
            "1) power mode is successfully set to Active, "
            "2) invalid set without 'mode' returns 400, "
            "3) power mode remains Active afterwards.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Step 0: Read current power mode (optional – for logging/restore)
        line = "[STEP] Reading current power mode via system/power-mode/get."
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(
            tester, device_id, "system/power-mode/get", "{}", logs, result
        )
        status = dab_status_from(response, rc)
        ...
        if status == 200:
            ...
            # Non-200 but not 501: we log and still attempt to proceed, but we cannot restore.
            line = f"[INFO] Could not determine original power mode; proceeding with test. status={status}"
            LOGGER.result(line)
            logs.append(line)

        # Step 1: Set power mode to "Active"
        payload_active = json.dumps({"mode": "Active"})
        line = f"[STEP] Setting power mode to 'Active' via system/power-mode/set, payload={payload_active}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(
            tester, device_id, "system/power-mode/set", payload_active, logs, result
        )
        status = dab_status_from(response, rc)
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/power-mode/get JSON parsing failed after setting 'Active': {e}"
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if mode_after_active != "Active":
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected power mode 'Active', but got {mode_after_active!r}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        payload_invalid = "{}"
        line = f"[STEP] Sending system/power-mode/set with missing 'mode' field, payload={payload_invalid}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(
            tester, device_id, "system/power-mode/set", payload_invalid, logs, result
        )
        status = dab_status_from(response, rc)
        line = f"[INFO] system/power-mode/set (missing 'mode') returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        if status == 501:
            # Inconsistent, but treat as not implemented for this payload.
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — system/power-mode/set reported status=501 for missing 'mode' payload."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if status != 400:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected status=400 for missing 'mode' field, but got status={status}."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 4: Confirm power mode is still 'Active'
        line = "[STEP] Confirming power mode remains 'Active' after invalid system/power-mode/set."
        LOGGER.result(line)
        logs.append(line)
        ...
        status = dab_status_from(response, rc)
        line = f"[INFO] system/power-mode/get after invalid set returned status={status}."
        LOGGER.result(line)
        logs.append(line)

        if status != 200:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/power-mode/get did not return 200 after invalid set; got {status}."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — system/power-mode/get JSON parsing failed after invalid set: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        final_mode = body_final.get("mode")
        line = f"[INFO] Power mode after invalid set: {final_mode!r}"
        LOGGER.result(line)
        logs.append(line)
        ...
        if final_mode != "Active":
            result.test_result = "FAILED"
            line = (
                f"[RESULT] FAILED — Power mode changed unexpectedly after invalid set. "
                f"Expected 'Active', observed {final_mode!r}."
            )
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        # All checks passed
        result.test_result = "PASS"
        line = "[RESULT] PASS — Missing 'mode' parameter correctly rejected with status=400 and power mode remained 'Active'."
        LOGGER.result(line)
        logs.append(line)

    except UnsupportedOperationError as e:
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running power-mode missing-parameter check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — Unexpected error during power-mode missing-parameter check: {e}"
        LOGGER.result(line)
        logs.append(line)

    finally:
        summary_line = (
            f"[SUMMARY] outcome={result.test_result}, original_mode={original_mode!r}, "
            f"final_mode={final_mode!r}, test_id={test_id}, device={device_id}"
        )
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_system_power_mode_active_to_standby_check(dab_topic, test_name, tester, device_id):
    ...
    Expected behavior:
      ...
      - DAB is still responsive during/after the transition.
    ...
    try:
        ...
        for line in (
            ...
            "verify via system/power-mode/get, and confirm DAB remains responsive.",
            "[DESC] Required operations: system/power-mode/get, system/power-mode/set.",
            "[DESC] Pass criteria: mode successfully becomes 'Standby' with status=200, and subsequent DAB calls succeed.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Step 1: Ensure precondition 'Active' (self-contained test)
        ...
        rc, response = execute_cmd_and_log(
            tester, device_id, "system/power-mode/set", payload_standby, logs, result
        )
        status = dab_status_from(response, rc)
        ...
        # Best-effort restore of original mode, if it was different and we know its value.
        try:
            if original_mode is not None and original_mode != "Standby":
                ...
                LOGGER.result(line)
                logs.append(line)
                rc_restore, _ = execute_cmd_and_log(
                    tester, device_id, "system/power-mode/set", payload_restore, logs, result
                )
        ...
        summary_line = f"[SUMMARY] outcome={result.test_result}, original_mode={original_mode!r}, test_id={test_id}, device={device_id}"
        LOGGER.result(summary_line)
        logs.append(summary_line)
...
def run_content_search_empty_query_behavior_check(dab_topic, test_name, tester, device_id):
    ...
    try:
        # Header / description
        for line in (
            f"[TEST] Content_Search Empty_Query Behavior Check — {test_name} (test_id={test_id}, device={device_id})",
            "[DESC] Goal: Verify that content/search with an empty searchText either returns a 400 error with valid JSON "
            "or a 200 response with entries=[].",
            "[DESC] Required operation: content/search.",
            "[DESC] Pass criteria: status in {400, 200}, valid JSON (if present), and for 200 we require entries=[].",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Step 1: Invoke content/search with empty searchText
        payload = json.dumps({"searchText": ""})
        line = f"[STEP] Invoking content/search with empty searchText, payload={payload}"
        LOGGER.result(line)
        logs.append(line)

        rc, response = execute_cmd_and_log(tester, device_id, "content/search", payload, logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] content/search empty-query returned status={status}"
        LOGGER.result(line)
        logs.append(line)
        ...
        if status == 501:
            result.test_result = "OPTIONAL_FAILED"
            line = "[RESULT] OPTIONAL_FAILED — content/search is not implemented on this device (status=501) for empty searchText."
            LOGGER.result(line)
            logs.append(line)
            return result

        # Only 200 or 400 are acceptable for this negative test
        if status not in (200, 400):
            result.test_result = "FAILED"
            line = (f"[RESULT] FAILED — Unexpected status for empty searchText: got {status}, "
                    "expected 400 (invalid params) or 200 (empty entries).")
            LOGGER.result(line)
            logs.append(line)
            return result

        # Step 2: Handle missing/empty body
        if response is None or response == "":
            if status == 400:
                result.test_result = "PASS"
                line = "[RESULT] PASS — content/search returned status=400 with no body for empty searchText; treated as valid client error."
                LOGGER.result(line)
                logs.append(line)
                return result
            else:
                result.test_result = "FAILED"
                line = "[RESULT] FAILED — content/search returned status=200 with an empty body; expected valid JSON even for empty entries."
                LOGGER.result(line)
                logs.append(line)
                return result
        ...
        try:
            ...
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — content/search response for empty searchText is not valid JSON: {e}"
            LOGGER.result(line)
            logs.append(line)
            return result

        # For status=400: any well-formed JSON is acceptable as an error payload
        if status == 400:
            LOGGER.result("[INFO] content/search empty-query returned status=400 with valid JSON body; treating as valid client error.")
            logs.append("[INFO] content/search empty-query returned status=400 with valid JSON body; treating as valid client error.")
            result.test_result = "PASS"
            line = "[RESULT] PASS — Empty searchText correctly rejected with status=400 and valid JSON."
            LOGGER.result(line)
            logs.append(line)
            return result

        # For status=200: must have entries list and it must be empty
        entries = body.get("entries")
        if not isinstance(entries, list):
            result.test_result = "FAILED"
            line = "[RESULT] FAILED — For status=200, content/search response does not expose 'entries' as a list for empty searchText."
            LOGGER.result(line)
            logs.append(line)
            return result
        ...
        if len(entries) != 0:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — For status=200, expected entries=[] for empty searchText, but got {len(entries)} entries."
            LOGGER.result(line)
            logs.append(line)
            return result

        # If we get here: status=200, JSON OK, entries=[]
        result.test_result = "PASS"
        line = "[RESULT] PASS — Empty searchText returned status=200 with valid JSON and entries=[], as allowed behavior."
        LOGGER.result(line)
        logs.append(line)

    except UnsupportedOperationError as e:
        result.test_result = "OPTIONAL_FAILED"
        line = f"[RESULT] OPTIONAL_FAILED — Operation '{e.topic}' is not supported while running content/search empty-query check."
        LOGGER.result(line)
        logs.append(line)

    except Exception as e:
        result.test_result = "SKIPPED"
        line = f"[RESULT] SKIPPED — Unexpected error during content/search empty-query behavior check: {e}"
        LOGGER.result(line)
        logs.append(line)
...
def run_system_power_mode_set_missing_mode_param_check(dab_topic, test_name, tester, device_id):
    ...
    Expected behavior:
      - Precondition: power mode is 'Active'.
      - Invalid call: system/power-mode/set with {} should return 400 (Bad Request).
      - Postcondition: system/power-mode/get still reports 'Active'.
      - 501 → OPTIONAL_FAILED.
    """
    test_id = to_test_id(f"{dab_topic}/{test_name}")
    logs = []
    ...
    try:
        ...
        for line in (
            ...
            "[DESC] Goal: Verify that system/power-mode/set rejects a request missing the 'mode' field and that the power state remains 'Active'.",
            "[DESC] Required operations: system/power-mode/get, system/power-mode/set.",
            "[DESC] Pass criteria: 1) power mode is set to 'Active', 2) invalid set without 'mode' returns 400,"
            " 3) power mode remains 'Active' afterwards.",
        ):
            LOGGER.result(line)
            logs.append(line)
        ...
        # Step 0: Read current power mode (for logging + best-effort restore)
        line = "[STEP] Reading current power mode via system/power-mode/get."
        LOGGER.result(line)
        logs.append(line)
        ...
        # Step 2: Send invalid system/power-mode/set without 'mode' field
        payload_invalid = "{}"
        line = f"[STEP] Sending system/power-mode/set with missing 'mode' field, payload={payload_invalid}"
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/set", payload_invalid, logs, result)
        status = dab_status_from(response, rc)
        line = f"[INFO] system/power-mode/set (missing 'mode') returned status={status}."
        LOGGER.result(line)
        logs.append(line)
        ...
        # Negative tests: expect 400 for bad request
        if status != 400:
            result.test_result = "FAILED"
            line = f"[RESULT] FAILED — Expected status=400 for missing 'mode' field, but got status={status}."
            LOGGER.result(line)
        ...
        # Step 3: Confirm power mode is still 'Active'
        line = "[STEP] Confirming power mode remains 'Active' after invalid system/power-mode/set."
        LOGGER.result(line)
        logs.append(line)
        rc, response = execute_cmd_and_log(tester, device_id, "system/power-mode/get", "{}", logs, result)
        ...
        # Best-effort restore to original mode if we changed it
        try:
            if original_mode is not None and original_mode != "Active":
                payload_restore = json.dumps({"mode": original_mode})
                line = f"[STEP] Best-effort restore: setting power mode back to original value {original_mode!r}."
                LOGGER.result(line)
                logs.append(line)
...
FUNCTIONAL_TEST_CASE = [
    ...
    ("system/setup/skip", "functional", run_system_setup_skip_mid_wizard_check, "System_Setup Skip_Mid_Wizard Check", "2.1", False),
    ("system/setup/skip", "functional", run_system_setup_skip_initial_wizard_check, "System_Setup Skip_Initial_Wizard Check", "2.1", False),
    ("system/power-mode/set", "functional", run_system_power_mode_set_missing_mode_param_check, "System_PowerMode_Set Missing_Mode_Param Check", "2.1", True),
    ...
    ("content/search","functional",run_content_search_empty_query_behavior_check,"Content_Search Empty_Query Behavior Check","2.1",True),
    ("system/power-mode/set","functional",run_system_power_mode_set_missing_mode_param_check,"System_PowerMode_Set Missing_Mode_Param Check","2.1",True),
    ("system/power-mode/set","functional",run_system_power_mode_active_to_standby_check,"System_PowerMode Active_To_Standby Check","2.1",False),
